      ...
      {show && show.episodes.length > 0 ? (
        <ul className="space-y-4">
          {show.episodes.map((episode) => (
            <li key={episode.title} className="border-b border-rule pb-7">
              ...
              <h3 className="mt-2 font-display text-xl font-bold leading-snug text-balance sm:text-2xl">
                {episode.link ? (
                  <a
                    href={episode.link}
                    target="_blank"
        ...
        <div className="border border-dashed border-rule-firm p-8">
          ...
          {PODCAST.platforms.map((platform) => (
            <li key={platform.href}>
              <a
                ...
                className="btn-ed-ghost !px-5 !py-2"
              >
                {platform.name}
              </a>
            </li>
